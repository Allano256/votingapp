...
def comfirm_selection(choice, candidate_data):
    """
    This requests the user to comfirm their choice or make changes.
    """
...
def update_candidate_votes(voter_choice, candidate_data):
    """
    This updates the votes for candidate A and B in the spreadsheet.
    """
...
def votes_cast():
    """
    This adds up the values from each candidate and gives a total
    """
    # Get the worksheet
    ...
    print(f"{total_votes_A} votes A and {total_votes_B}votes B")
...
def main():
    """
    This gets called called when the application starts running.
    """
    candidate_worksheet = SHEET.worksheet('candidate')
    candidate_data = candidate_worksheet.get_all_records()
    get_voter_choice(candidate_data)
